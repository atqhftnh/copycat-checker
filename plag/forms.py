...
class ClassroomForm(forms.ModelForm):
    ...
    class Meta:
        model = Classroom
        # 'join_code' is NOT in fields because it's auto-generated by the model's save method
        # and not expected as user input from the HTML modal.
    ...
    def clean(self):
        ...
        # Get the lecturer from the request.user, ensuring it's a lecturer.
        if self.request and hasattr(self.request.user, 'is_lecturer') and self.request.user.is_lecturer:
            current_lecturer = self.request.user
        elif self.instance.pk:
            # If we're updating an existing instance, get the lecturer from that instance
            current_lecturer = self.instance.lecturer

        # --- Duplicate Check: Name + Group for THIS specific Lecturer ---
        if name and group and current_lecturer: # Ensure all necessary data is present
            # Query for existing classrooms by the same lecturer with the same name AND group
            query_name_group = Classroom.objects.filter(
                name=name,
                group=group,
                lecturer=current_lecturer # Use the identified lecturer
            )
            if self.instance.pk: # Exclude the current instance if it's an update operation
                query_name_group = query_name_group.exclude(pk=self.instance.pk)

            if query_name_group.exists():
                self.add_error(
                    'name',
                    "You already have a classroom with this name and group. Please change the name or group."
                )
                self.add_error(
                    'group',
                    "You already have a classroom with this name and group. Please change the name or group."
        ...
        # No join_code validation here, as it's auto-generated by the model
